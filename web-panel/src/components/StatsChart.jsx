...
export function StatsChart({ logs, isDarkMode, isAdmin }) {
  ...
  if (logs.length === 0) {
    return <div className=" min-h-[140px] text-center p-4 md:p-6 text-zinc-400 dark:text-zinc-600 text-xs transition-colors">Sin datos estadísticos</div>;
  }

  return (
    <div className={`w-full ${!isAdmin ? 'max-md:py-6 md:aspect-video' : 'py-6'} bg-zinc-50 dark:bg-zinc-950/40 border border-zinc-200 dark:border-zinc-800 rounded-xl p-4 md:p-6 flex flex-col xl:flex-row items-center justify-center gap-6 transition-colors min-h-[140px]`}>
