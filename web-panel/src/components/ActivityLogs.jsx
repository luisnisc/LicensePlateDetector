    ...
    <div>
      <div className="mb-6 flex justify-between items-center">
        <div>
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1 transition-colors">{titleLog}</p>
        </div>
        <div className="flex items-center gap-3">
          ...
          {isAdmin && (
            <button
              ...
              className="text-xs font-semibold px-3 py-1.5 rounded-xl uppercase tracking-wider bg-red-50 dark:bg-red-400/10 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-400/20 hover:bg-red-100 dark:hover:bg-red-400/20 transition-colors cursor-pointer"
            >
              Limpiar
            </button>
          )}
      ...
      {logs.length === 0 ? (
        <div className="min-h-[350px] text-center py-12 text-zinc-400 dark:text-zinc-500 text-sm border border-dashed border-zinc-300 dark:border-zinc-800 rounded-xl h-48 flex items-center justify-center transition-colors">
          Esperando detecciones...
        </div>
      ) : (
        <ul className="max-h-[350px] min-h-[350px] overflow-y-auto flex flex-col gap-2 pr-1 custom-scrollbar">
          {logs.map((item, index) => {
